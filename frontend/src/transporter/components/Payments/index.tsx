...
import { ChevronRight, DollarSign, IndianRupee, Clock, CheckCircle, ShieldCheck, Truck, FileText } from 'lucide-react';
...
type SectionTab = 'permits' | 'trucks';
...
export function Payments({ onNavigateToPermit }: PaymentsProps) {
  const { permits, loads, trucks, loading, refreshAll } = useSync();
  const [activeSection, setActiveSection] = useState<SectionTab>('permits');
  const [selectedPermit, setSelectedPermit] = useState<string | null>(null);

  /* ── Build permit summaries ──────────── */
  const permitSummaries = useMemo<PermitSummary[]>(() => {
  ...
  const truckSummaries = useMemo<TruckSummary[]>(() => {
    ...
    paymentLoads.forEach(l => {
      ...
      entry.loadCount++;
      if (l.hasFlag) entry.disputeLoads++;
      else entry.paidLoads++;
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <div className="flex gap-1 bg-neutral-100 rounded-lg p-1 mb-6 w-fit">
        {([
          { id: 'permits' as SectionTab, label: 'Permits', icon: <FileText size={15} />, count: stats.totalPermits },
          { id: 'trucks' as SectionTab, label: 'Trucks', icon: <Truck size={15} />, count: stats.totalTrucks },
        ]).map(tab => (
          <button
            key={tab.id}
