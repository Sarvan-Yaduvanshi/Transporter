...
import { createTruck, updateTruck, deleteTruck, type Truck } from '@/services/api';
...
interface TruckForm { truckNumber: string; owner: string; driver: string; status: string; availabilityWindow: string }
const emptyForm: TruckForm = { truckNumber: '', owner: '', driver: '', status: 'Available', availabilityWindow: '' };
...
export function TrucksView() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingTruck) {
        await updateTruck(editingTruck, { owner: form.owner, driver: form.driver, status: form.status, availabilityWindow: form.availabilityWindow } as any);
      } else {
  ...
  const handleDelete = async (truckNumber: string) => {
    if (!confirm(`Delete truck ${truckNumber}? This cannot be undone.`)) return;
    try { await deleteTruck(truckNumber); refreshAll(); } catch (e: any) { alert(e.message || 'Failed to delete'); }
  };
