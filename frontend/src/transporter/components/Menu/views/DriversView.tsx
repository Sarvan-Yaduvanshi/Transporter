...
import {
  ...
  type Driver, type DriverDocument,
} from '@/services/api';
...
interface DriverForm {
  ...
  assignedTruck: string; status: string; address: string; emergencyContact: string;
}
...
interface DocForm {
  ownerType: 'Driver' | 'Truck'; ownerId: string; docType: string;
  docNumber: string; issueDate: string; expiryDate: string; status: string; notes: string;
}
const emptyDocForm = (ownerType: 'Driver' | 'Truck', ownerId: string): DocForm => ({
...
export function DriversView() {
  ...
  const handleDelete = async (licenseNumber: string) => {
    if (!confirm(`Delete this driver and all their documents? This cannot be undone.`)) return;
    try { await deleteDriver(licenseNumber); refreshAll(); } catch (e: any) { alert(e.message || 'Failed to delete'); }
  };
...
/* ═══════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════ */
function DriverDetailView({ driver, onBack }: { driver: Driver; onBack: () => void }) {
  const [tab, setTab] = useState<'personal' | 'truck'>('personal');
...
function DocumentsPanel({ ownerType, ownerId, docTypeOptions, truckLabel }: {
  ownerType: 'Driver' | 'Truck'; ownerId: string; docTypeOptions: string[]; truckLabel?: string;
}) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await deleteDocument(id); refreshAll(); } catch (e: any) { alert(e.message || 'Failed to delete'); }
  };
