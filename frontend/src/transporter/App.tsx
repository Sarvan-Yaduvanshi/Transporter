...
export type NavigationTab = 'home' | 'allocation' | 'payments' | 'menu';
export interface ViewState { tab: NavigationTab; permitView?: string; showProfile?: boolean; }

const NAV_ITEMS = [
    { id: 'home' as NavigationTab, label: 'Live Operations', icon: Activity },
    { id: 'allocation' as NavigationTab, label: 'Allocation', icon: Package },
    { id: 'payments' as NavigationTab, label: 'Payments', icon: DollarSign },
    { id: 'menu' as NavigationTab, label: 'Menu', icon: MenuIcon },
];
...
interface TransporterAppProps {
    onSwitchToDriver?: () => void;
}

export default function TransporterApp({ onSwitchToDriver }: TransporterAppProps) {
    const { user, logout } = useAuth();
    const [viewState, setViewState] = useState<ViewState>({ tab: 'home' });
    const navigateToTab = (tab: NavigationTab) => setViewState({ tab });
    const navigateToPermit = (permitNumber: string) => setViewState({ tab: viewState.tab, permitView: permitNumber });
