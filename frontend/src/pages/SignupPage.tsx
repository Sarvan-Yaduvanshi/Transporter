...
/* ── OAuth trigger helpers (same as LoginPage) ──── */

async function triggerGoogleOAuth(): Promise<{ googleId: string; name: string; email: string; avatar?: string }> {
        ...
        return new Promise((resolve, reject) => {
            (window as any).google.accounts.id.initialize({
                ...
                callback: (response: any) => {
                    ...
                    resolve({ googleId: payload.sub, name: payload.name, email: payload.email, avatar: payload.picture });
                },
            });
            (window as any).google.accounts.id.prompt((notification: any) => {
                if (notification.isNotDisplayed() || notification.isSkippedMoment()) reject(new Error('Google sign-in was cancelled'));
            });
        });
    }
    throw new Error('Google OAuth SDK not loaded. Add your VITE_GOOGLE_CLIENT_ID to .env and include the GSI script.');
}

async function triggerFacebookOAuth(): Promise<{ facebookId: string; name: string; email: string; avatar?: string }> {
    if (typeof window !== 'undefined' && (window as any).FB) {
        return new Promise((resolve, reject) => {
            (window as any).FB.login((response: any) => {
                if (response.authResponse) {
                    (window as any).FB.api('/me', { fields: 'id,name,email,picture' }, (user: any) => {
                        resolve({ facebookId: user.id, name: user.name, email: user.email, avatar: user.picture?.data?.url });
                    });
                } else {
    ...
    throw new Error('Facebook SDK not loaded. Add your VITE_FACEBOOK_APP_ID to .env and include the FB SDK script.');
}
