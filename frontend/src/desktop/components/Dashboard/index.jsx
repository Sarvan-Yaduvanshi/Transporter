...
export function Dashboard({ onViewLoad, onViewPermit }) {
  ...
  const statCards = [
  { value: totalTrucks, label: 'Total Trucks', sub: 'in fleet', color: 'text-slate-500' },
  { value: onLoad, label: 'On Load', sub: 'currently active', color: 'text-blue-600', dot: true },
  { value: idle > 0 ? idle : 0, label: 'Idle', sub: 'available', color: 'text-emerald-600', dot: true },
  { value: maintenance, label: 'Maintenance', sub: 'under service', color: 'text-amber-600', dot: true }];
...
function LoadCard({ truck, permit, onClick, done, flagged }) {
  return (
    <button
      onClick={onClick}
...
function SectionLabel({ label, red }) {
  return (
