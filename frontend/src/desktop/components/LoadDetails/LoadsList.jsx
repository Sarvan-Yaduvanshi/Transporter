...
import {
  Search, AlertTriangle, Truck,
  Package, ArrowRight, Loader2, ArrowDownToLine, Banknote,
  MapPin, Hash, ArrowUpRight, Plus, Pencil, Trash2 } from
'lucide-react';
...
export function LoadsList({ onViewLoad, onViewInvoice }) {
  ...
  const getPermit = (permitNumber) => permits.find((p) => p.permitNumber === permitNumber) ?? null;

  const allLoads = loadsData ?? [];
  const showToast = (msg) => {setToast(msg);setTimeout(() => setToast(null), 3000);};

  const openCreate = () => {setEditingLoad(null);setShowForm(true);};
  const openEdit = (load) => {setEditingLoad(load);setShowForm(true);};
  const closeForm = () => {setShowForm(false);setEditingLoad(null);};
  ...
  const handleDelete = async (loadId) => {
    ...
    try {
      ...
      showToast(e.message || 'Failed to delete load');
    }
  };
      ...
      <div className="fixed top-6 right-6 z-50 bg-neutral-900 text-white px-5 py-3 rounded-2xl shadow-2xl text-sm font-medium backdrop-blur-sm border border-neutral-700">
            ...
            <div className="flex items-end justify-between mb-10">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button
                        onClick={openCreate}
                        className="flex items-center gap-2 px-4 py-2.5 bg-neutral-900 text-white text-sm font-semibold rounded-2xl hover:bg-neutral-800 transition-colors">
