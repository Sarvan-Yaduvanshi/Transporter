...
import {
    ...
    FileText, ChevronRight, ShieldCheck, Package, ArrowRight,
    Loader2, ArrowDownToLine, Banknote, Send, Hash, ArrowUpRight, ShieldAlert,
} from 'lucide-react';
...
const FLOW_META: Record<FlowStep, { label: string; color: string; bg: string; border: string; icon: any; gradient: string; accent: string }> = {
...
function FlowBar({ currentStep }: { currentStep: FlowStep }) {
...
export function LoadDetails({ loadId, onBack, onViewInvoice }: LoadDetailsProps) {
    ...
    const handleSendForApproval = async () => {
        ...
        } catch { showToast('Failed to send for approval'); }
        finally { setUpdating(false); }
    };

    /* ── Contextual hint texts ──────────── */
    const flowHints: Record<FlowStep, string> = {
