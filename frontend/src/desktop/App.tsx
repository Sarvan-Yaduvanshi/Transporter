...
import { Sidebar, type DesktopPage } from './components/Layout/Sidebar';
...
interface DesktopViewState {
    page: DesktopPage;
    loadId?: string | null;
    invoicePermit?: string | null;
    permitNumber?: string | null;
    showProfile?: boolean;
}

const PAGE_LABELS: Record<DesktopPage, string> = {
...
export default function DriverApp({ onSwitchToTransporter }: DriverAppProps) {
    const [view, setView] = useState<DesktopViewState>({ page: 'dashboard' });

    /* ── Navigation helpers ────────────────────────────── */
    const navigateTo = useCallback((page: DesktopPage) => {
        setView({ page });
    }, []);
    ...
    /* ── Breadcrumbs ──────────────────────────────────── */
    const breadcrumbs: Breadcrumb[] = [{ label: PAGE_LABELS[view.page], onClick: view.loadId || view.invoicePermit || view.permitNumber || view.showProfile ? () => navigateTo(view.page) : undefined }];
